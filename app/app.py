...
if page == "Overview":
    ...
    if fleet is None or anom is None:
        ...
        st.subheader("Current RUL and Health")
        col1, col2 = st.columns(2)
        if not ruleng.empty:
            latest_rul = ruleng.iloc[-1]
            rul_val = latest_rul.get("overall_RUL_cycles", latest_rul.get("ensemble_RUL_cycles", "N/A"))
            health_val = latest_rul.get("overall_health_score", latest_rul.get("ensemble_health_score", "N/A"))
            
            if pd.notna(rul_val) and isinstance(rul_val, (int, float)):
                col1.metric("Remaining Useful Life (RUL)", f"{rul_val:.0f} cycles")
            else:
                col1.metric("Remaining Useful Life (RUL)", str(rul_val))
                
            if pd.notna(health_val) and isinstance(health_val, (int, float)):
                col2.metric("Health Score", f"{health_val:.0f} %")
            else:
                col2.metric("Health Score", str(health_val))
        else:
            st.info("No RUL data available for this engine.")
