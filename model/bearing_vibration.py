...
def derive_operating_state(row: pd.Series) -> Dict[str, float]:
    ...
    throttle_proxy = float(np.clip(
        65.0 + 35.0 * (0.55 * _safe_scale(nc, 9020.0, 9150.0)
                        + 0.45 * _safe_scale(t30, 1575.0, 1610.0)),
        65.0, 100.0,
    ))
    load = float(np.clip(
        0.45 * _safe_scale(p30, 550.0, 555.0)
        + 0.35 * _safe_scale(t30, 1575.0, 1610.0)
...
def _degradation_proxy(row: pd.Series, group: pd.DataFrame | None = None) -> float:
    ...
    nc = float(row.get("Nc", 9040.0))
    t30 = float(row.get("T30", 1580.0))
    p30 = float(row.get("P30", 552.0))
    base = 0.35 * _safe_scale(t30, 1575.0, 1605.0) + 0.25 * _safe_scale(p30, 551.0, 554.5)
    speed_stress = 0.20 * _safe_scale(nc, 9040.0, 9150.0)
